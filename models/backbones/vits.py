...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)  # patch嵌入，相当于Conv2d(in_chans, embed_dim, kernel_size=(16,16), stride=(16,16))

        num_patches = self.patch_embed.num_patches

        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))  # CLS信息聚合器 维度为 (1, 1, embed_dim)，并用全零张量进行初始
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))  # 位置编码
        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth decay rule
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]
        # torch.linspace返回一个张量，这个张量包含了从0到drop_path_rate等距的depth个点
    ...
    def forward(self, x, register_blk=-1):
        ...
        cls_tokens = self.cls_token.expand(B, -1, -1)  # 将CLS复制B份堆叠起来
        x = torch.cat((cls_tokens, x), dim=1)  # 拼接

        x = x + self.pos_embed[:, :x.size(1), :]  #位置编码
